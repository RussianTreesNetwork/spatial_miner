...
public class BatteryTileEntity extends BlockEntity implements IEnergyStorage {
    ...
    public BatteryTileEntity(BlockPos pos, BlockState state) {
        super(ModTileEntities.BATTERY_TILE.get(), pos, state);
        this.capacity = ConfigManager.batteryCapacity;
        this.energy = 0;
    }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, BatteryTileEntity battery) {
        if (level.isClientSide()) return;

        // Здесь будет логика передачи энергии в сеть
        battery.transferEnergyToNetwork();
    }

    private void transferEnergyToNetwork() {
        // Логика передачи энергии через кабельную сеть
